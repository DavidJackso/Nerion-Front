...
// ── View state ─────────────────────────────────────────────────
const view = ref<PdfViewState>('templates')
const activeTpl = ref<PdfTemplate | null>(null)

// Real templates & archive
const templates = ref<PdfTemplate[]>([])
const archive = ref<PdfJob[]>([])
...
const LIB_TPLS = [
  ...
  { name: 'Трудовой договор', cat: 'hr', gost: 'ТК РФ ст. 57', desc: 'Типовой трудовой договор с полями для реквизитов сторон.', official: true },
  { name: 'Приказ о приёме', cat: 'hr', gost: 'Унифор. форма Т-1', desc: 'Форма Т-1 для оформления приёма сотрудника на работу.', official: true },
  { name: 'Акт выполненных работ', cat: 'finance', gost: 'ГК РФ ст. 720', desc: 'Двусторонний акт с подписями заказчика и исполнителя.', official: false },
  { name: 'Счёт-фактура', cat: 'finance', gost: 'НК РФ ст. 169', desc: 'Налоговая счёт-фактура согласно требованиям НК РФ.', official: true },
  { name: 'Коммерческое предложение', cat: 'finance', gost: '', desc: 'Шаблон КП с логотипом, позициями и итогами.', official: false },
  { name: 'Диплом о высшем образовании', cat: 'edu', gost: 'Пр. Минобр. № 636', desc: 'Официальный бланк диплома ВО нового образца.', official: true },
  { name: 'Справка об обучении', cat: 'edu', gost: 'ГОСТ Р 7.0.97-2016', desc: 'Справка для студентов очной/заочной формы обучения.', official: true },
  { name: 'Договор оказания услуг', cat: 'legal', gost: 'ГК РФ гл. 39', desc: 'Стандартный договор возмездного оказания услуг.', official: false },
  { name: 'Доверенность', cat: 'legal', gost: 'ГК РФ ст. 185', desc: 'Нотариально удостоверяемая доверенность на представительство.', official: true },
...
const mapping = ref<FieldMapping[]>([
  { ph: 'ФИО', label: 'ФИО', pdf_field: 'ФИО', table_column: '', table: '', field: '', fmt: '—', sample: '—' },
  { ph: 'ДОЛЖНОСТЬ', label: 'Должность', pdf_field: 'ДОЛЖНОСТЬ', table_column: '', table: '', field: '', fmt: '—', sample: '—' },
  { ph: 'КАФЕДРА', label: 'Кафедра / отдел', pdf_field: 'КАФЕДРА', table_column: '', table: '', field: '', fmt: '—', sample: '—' },
  { ph: 'ДАТА_НАЙМА', label: 'Дата найма', pdf_field: 'ДАТА_НАЙМА', table_column: '', table: '', field: '', fmt: 'DD.MM.YYYY', sample: '—' },
  { ph: 'СТАВКА', label: 'Ставка', pdf_field: 'СТАВКА', table_column: '', table: '', field: '', fmt: '—', sample: '—' },
  { ph: 'ПОДПИСАНТ', label: 'Подписант', pdf_field: 'ПОДПИСАНТ', table_column: '', table: '', field: '', fmt: '—', sample: '—' },
  { ph: 'ДАТА_ВЫД', label: 'Дата выдачи', pdf_field: 'ДАТА_ВЫД', table_column: '', table: '', field: '', fmt: 'DD.MM.YYYY', sample: '—' },
  { ph: 'НОМЕР', label: 'Номер документа', pdf_field: 'НОМЕР', table_column: '', table: '', field: '', fmt: '—', sample: '—' },
])
const zoom = ref(78)
...
const breadcrumb = computed(() => {
  const map: Record<PdfViewState, string[]> = {
    templates: ['PDF', 'Мои шаблоны'],
    library:   ['PDF', 'Библиотека'],
    archive:   ['PDF', 'Архив'],
    editor:    ['PDF', tplName(activeTpl.value) || 'Шаблон', 'Настройка'],
    generate:  ['PDF', tplName(activeTpl.value) || 'Шаблон', 'Генерация'],
  }
  return [space.value?.name || spaceSlug.value, ...(map[view.value] || [])]
})
...
const filteredLib = computed(() =>
  LIB_TPLS.filter(t =>
    (libCat.value === 'all' || t.cat === libCat.value) &&
    (!libSearch.value || t.name.toLowerCase().includes(libSearch.value.toLowerCase()))
  )
)
...
const filteredArchive = computed(() =>
  archive.value.filter(a =>
    !archiveSearch.value || String(jobId(a)).includes(archiveSearch.value)
  )
)
...
const bindOptions = computed(() => {
  ...
  schemaStore.tables.forEach((tbl: any) => {
    groups.push({
      group: tbl.slug,
      items: (tbl.fields || []).map((f: any) => ({
        value: `${tbl.slug}::${f.slug || f.name}`,
        label: f.name || f.slug,
      })),
    })
  })
...
// ── Helpers ────────────────────────────────────────────────────
function tplName(t: PdfTemplate | null): string {
  return t?.Name || t?.name || ''
}

function tplId(t: PdfTemplate | null): string {
  return t?.ID || t?.id || ''
}

function tplStatus(t: PdfTemplate): string {
  return t.Status === 'ready' ? 'ready' : 'setup'
}

function tplStatusLabel(t: PdfTemplate): string {
  return t.Status === 'ready' ? 'Готов' : 'Настройка'
}

function tplStatusTone(t: PdfTemplate): string {
  return t.Status === 'ready' ? 'success' : 'warning'
}

function phLabel(ph: string): string {
  return '{{' + ph + '}}'
}

function jobId(a: PdfJob): string {
  return String(a.ID || a.id || '')
}

function jobStatus(a: PdfJob): string {
  return (a.Status || a.status || '').toLowerCase()
}

function jobStatusTone(a: PdfJob): string {
  const s = jobStatus(a)
  return s === 'done' ? 'success' : s === 'error' ? 'danger' : 'neutral'
}

function archiveName(a: PdfJob): string {
  const path = a.StoragePath || a.output_url || ''
  return path ? path.split('/').pop() || `job_${jobId(a)}` : `job_${jobId(a)}`
}

function jobProcessed(a: PdfJob): number {
...
onMounted(async () => {
  ...
  try {
    await Promise.all([
      listTemplates(spaceSlug.value).then((r: PdfTemplate[]) => { templates.value = r }),
      schemaStore.fetchTables(spaceSlug.value),
    ])
  } catch (e: any) {
    showToast(e.message)
  } finally {
...
watch(view, async (v) => {
  if (v === 'archive' && !archive.value.length) {
    ...
    try {
      archive.value = await listArchive(spaceSlug.value)
    } catch (e: any) {
      showToast(e.message)
    } finally {
...
// ── Methods ────────────────────────────────────────────────────
function setMapRow(i: number, patch: Partial<FieldMapping>) {
  mapping.value = mapping.value.map((m, j) => j === i ? { ...m, ...patch } : m)
}

function onBindChange(i: number, val: string) {
  const [table, field] = val.split('::')
  setMapRow(i, {
    table: table || '',
    field: field || '',
    table_column: val,
    sample: field ? mapping.value[i].sample : '—',
  })
}
...
function openGenerate(tpl: PdfTemplate) {
  activeTpl.value = tpl
  genPhase.value = 'idle'
  genProgress.value = 0
  genJobId.value = null
...
async function doSaveMapping() {
  try {
    await saveMapping(spaceSlug.value, tplId(activeTpl.value), mapping.value.map(m => ({
      placeholder: m.ph,
...
async function startGenerate() {
  const recordIds = genRecordIds.value
    .split(',')
    .map(s => parseInt(s.trim()))
    .filter(n => !isNaN(n))
  if (!genTableSlug.value) { showToast('Выбери таблицу'); return }
  genPhase.value = 'running'
  genProgress.value = 0
  try {
    const job = await generatePDF(spaceSlug.value, {
      template_id: tplId(activeTpl.value),
      table_slug: genTableSlug.value,
      record_ids: recordIds,
    })
    genJobId.value = job.ID || job.id
    pollJob()
  } catch (e: any) {
...
function pollJob() {
  ...
  genPollTimer = setInterval(async () => {
    try {
      const job: PdfJob = await getJob(spaceSlug.value, genJobId.value!)
      if (job.TotalRecords && job.TotalRecords > 0) {
        genProgress.value = Math.round(((job.Processed ?? 0) / job.TotalRecords) * 100)
      }
...
async function doUploadTemplate() {
  ...
  try {
    const tpl: PdfTemplate = await uploadTemplate(spaceSlug.value, uploadName.value.trim(), uploadFile.value)
...
<template>
  <div>
    ...
    >
      <button
        v-for="[k, l] in [['templates','Мои шаблоны'],['library','Библиотека'],['archive','Архив']]"
        :key="k"
        @click="view = k as PdfViewState"
        :style="{
      ...
      <div style="display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 24px">
        <div>
          ...
          <p style="font-size: 13px; color: var(--fg-2)">
            {{ templates.length }} шаблонов · {{ templates.filter(t => t.Status === 'ready').length }} готовы
          </p>
        </div>
        ...
        >
          ...
          <div style="flex: 1; min-width: 0; display: flex; flex-direction: column">
            <div style="display: flex; align-items: flex-start; gap: 8px">
              <div style="font-size: 14px; font-weight: 600; flex: 1; line-height: 1.3">{{ tplName(t) }}</div>
      ...
      <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px; flex-wrap: wrap">
        <div style="position: relative">
          ...
          <input
            ...
            style="height: 36px; width: 280px; padding: 0 12px 0 32px; border: 0.5px solid var(--border-strong); border-radius: 6px; background: var(--bg-0); font-size: 13px; outline: 0; font-family: inherit"
          />
        </div>
        ...
        >
          ...
          <div style="padding: 14px">
            <div style="display: flex; align-items: flex-start; gap: 6px; margin-bottom: 4px">
              ...
              <div
                v-if="t.official"
                style="width: 14px; height: 14px; border-radius: 50%; background: var(--green-100); color: var(--green-700); display: grid; place-items: center; flex-shrink: 0"
              >
    ...
    <div v-else-if="view === 'editor'" style="display: flex; flex-direction: column; height: calc(100vh - 56px)">
      ...
      <div style="height: 46px; background: var(--bg-0); border-bottom: 0.5px solid var(--border-default); display: flex; align-items: center; padding: 0 20px; gap: 14px; font-size: 12px; flex-shrink: 0">
        <button
          @click="view = 'templates'"
          style="background: 0; border: 0; color: var(--fg-2); font-size: 12px; cursor: pointer; display: flex; align-items: center; gap: 5px; padding: 0; font-family: inherit"
        >
        ...
        <div style="flex: 1"></div>
        <NButton variant="ghost" size="sm" @click="doSaveMapping">
      ...
      <div style="display: grid; grid-template-columns: 440px 1fr; flex: 1; min-height: 0">
        ...
        <aside style="background: var(--bg-0); border-right: 0.5px solid var(--border-default); display: flex; flex-direction: column; overflow: hidden">
          ...
          <div
            v-if="showBanner"
            style="background: linear-gradient(90deg,var(--brand-tint),#F8F4FF); border-bottom: 0.5px solid var(--purple-200); padding: 12px 18px; display: flex; align-items: flex-start; gap: 10px"
          >
            ...
            >
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px">
                ...
                <NSelect
                  ...
                  size="sm"
                  :options="['—','DD.MM.YYYY','YYYY-MM-DD','число','текст'].map(f => ({ value: f, label: f === '—' ? 'без формата' : f }))"
                />
              </div>
        ...
        <!-- Right: live ГОСТ preview with zoom controls -->
    ...
    <div v-else-if="view === 'generate'" style="display: grid; grid-template-columns: 420px 1fr; height: calc(100vh - 56px)">
      ...
      <div style="border-right: 0.5px solid var(--border-default); background: var(--bg-1); display: flex; flex-direction: column; overflow: hidden">
        <div style="padding: 18px 24px 14px; border-bottom: 0.5px solid var(--border-default); background: var(--bg-0)">
          <button
            @click="view = 'templates'"
            style="background: 0; border: 0; color: var(--fg-3); font-size: 12px; cursor: pointer; display: flex; align-items: center; gap: 6px; margin-bottom: 12px; padding: 0; font-family: inherit"
          >
          ...
          <div style="display: flex; align-items: center; gap: 12px">
            ...
            <div>
              <h1 style="font-size: 16px; font-weight: 700">{{ tplName(activeTpl) || 'Шаблон' }}</h1>
            </div>
          </div>
        </div>
        ...
        <div style="flex: 1; overflow: auto; padding: 18px 24px">
          ...
          <NSelect
            ...
            :options="schemaStore.tables.map((t: any) => ({ value: t.slug, label: t.name || t.slug }))"
          ...
          <div
            v-if="genTableSlug && genRecordIds.trim()"
            style="display: flex; align-items: center; gap: 8px; padding: 10px 12px; background: var(--brand-tint); border-radius: 6px; font-size: 12px; color: var(--purple-700)"
          >
    ...
    <div v-else-if="view === 'archive'" style="max-width: 1000px; margin: 0 auto; padding: 28px 32px 80px">
      ...
      <div style="position: relative; margin-bottom: 16px; max-width: 320px">
        ...
        <input
          ...
          style="height: 36px; width: 100%; padding: 0 12px 0 32px; border: 0.5px solid var(--border-strong); border-radius: 6px; background: var(--bg-0); font-size: 13px; outline: 0; font-family: inherit; box-sizing: border-box"
        />
      </div>
      ...
      <div v-else style="background: var(--bg-0); border: 0.5px solid var(--border-default); border-radius: 8px; overflow: hidden">
        ...
        >
          ...
          <div style="font-size: 13px; color: var(--fg-2)">
            <NBadge :tone="jobStatusTone(a)" :dot="true">{{ jobStatus(a) }}</NBadge>
          </div>
          <div style="font-size: 13px; color: var(--fg-2)">{{ jobProcessed(a) }} записей</div>
          <div style="font-size: 13px; color: var(--fg-2); white-space: nowrap">{{ fmtDate(a.CreatedAt || a.created_at) }}</div>
          <div style="display: flex; gap: 2px; justify-content: flex-end">
            <button
              @click="showToast('Скачивание начато')"
              style="background: 0; border: 0; padding: 5px; cursor: pointer; color: var(--fg-2); display: flex; border-radius: 4px"
            >
              <NIcon name="download" :size="14" />
            </button>
            <button
              @click="showToast('Ссылка скопирована')"
              style="background: 0; border: 0; padding: 5px; cursor: pointer; color: var(--fg-2); display: flex; border-radius: 4px"
            >

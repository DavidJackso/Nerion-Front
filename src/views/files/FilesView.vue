...
onMounted(async () => {
  ...
  try {
    await Promise.all([
      ...
      listsApi.listLists(slug.value).then(r => { lists.value = r }),
    ])
  } catch (e) {
    show(e.message)
  } finally {
...
async function createList() {
  ...
  try {
    const l = await listsApi.createList(slug.value, {
...
async function togglePublished(l) {
  try {
    const updated = await listsApi.updateList(slug.value, l.slug, { published: !l.published })
...
function listEndpoint(l) {
  return `/lists/${slug.value}/${l.slug}`
}

function listCodeSnippet(l) {
...
function heroCodeSnippet() {
  ...
  if (!first) return `const res = await fetch('https://app.nerion.ru/lists/${slug.value}/my-list');
const { data } = await res.json();`
...
function tableName(tableSlug) {
  return schemaStore.tables.find(t => t.slug === tableSlug)?.name || tableSlug
}
...
<template>
  <AppShell :breadcrumb="crumb">
    ...
    <div v-if="activeList" style="padding:20px 32px 56px;max-width:1080px">
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px">
        ...
        <NBadge :tone="activeList.published ? 'success' : 'neutral'" :dot="true">{{ activeList.published ? 'публичный' : 'по ключу' }}</NBadge>
      </div>
      <p style="font-size:13px;color:var(--fg-2);margin-bottom:18px">
        Список публикует записи таблицы <strong>{{ tableName(activeList.table_slug) }}</strong> по постоянному адресу. Фронт забирает их без знания ID.
      </p>
      ...
      <div style="margin-bottom:28px">
        ...
        <div style="position:relative;background:#111827;border-radius:8px;overflow:hidden">
          <button @click="copyCode(listCodeSnippet(activeList))" style="position:absolute;top:8px;right:8px;background:rgba(255,255,255,.08);border:0;color:rgba(255,255,255,.7);padding:4px 9px;border-radius:4px;cursor:pointer;font-size:11px;display:flex;align-items:center;gap:4px;font-family:inherit;z-index:1">
      ...
      <div style="background:var(--bg-0);border:0.5px solid var(--border-default);border-radius:8px;padding:16px 20px">
        <div style="display:grid;grid-template-columns:140px 1fr;gap:8px 16px;font-size:13px">
          <div style="color:var(--fg-3)">Таблица</div><div style="color:var(--fg-1);font-weight:500">{{ tableName(activeList.table_slug) }}</div>
          <div style="color:var(--fg-3)">Slug</div><div><code style="font-family:var(--font-mono);font-size:12px;color:var(--fg-1)">{{ activeList.slug }}</code></div>
          <div style="color:var(--fg-3)">Лимит записей</div><div style="color:var(--fg-1)">{{ activeList.row_limit || 50 }}</div>
          <div style="color:var(--fg-3)">Создан</div><div style="color:var(--fg-2)">{{ fmtDate(activeList.created_at) }}</div>
    ...
    <div v-else-if="tab === 'lists'" style="padding:20px 32px 56px;max-width:1080px">
      ...
      <div style="background:linear-gradient(90deg,var(--brand-tint),#F8F4FF);border:0.5px solid var(--purple-200);border-radius:12px;padding:18px;margin-bottom:22px">
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:12px">
          ...
          <NButton v-if="lists.length" variant="primary" size="md" @click="activeList = lists[0]">Открыть список</NButton>
        </div>
      ...
      <template v-else>
        ...
        <div v-if="!lists.length" style="padding:40px;text-align:center;background:var(--bg-0);border:0.5px solid var(--border-default);border-radius:10px">
        ...
        <div v-else style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:12px">
          ...
          >
            <div style="display:flex;align-items:flex-start;gap:12px">
              ...
              <NBadge :tone="l.published ? 'success' : 'neutral'" :dot="true">{{ l.published ? 'публичный' : 'скрыт' }}</NBadge>
            </div>
  ...
  <!-- Create list modal -->
  <NModal :open="showCreate" @close="showCreate = false; createError = ''" title="Новый список" subtitle="Публикует записи таблицы по постоянному адресу." :width="480">
    <div style="display:flex;flex-direction:column;gap:16px">
      <div>
        ...
        <NSelect
          ...
          @update:model-value="onTablePick($event)"
          placeholder="— выбери таблицу —"
          :options="schemaStore.tables.map(t => ({ value: t.slug, label: t.name }))"
        />
      </div>
      <div>
      ...
      <div>
        ...
        <div style="display:flex;gap:8px">
          <button v-for="[v, t, h] in [[true,'Публичный','Без ключа'],[false,'По ключу','Bearer-токен']]" :key="String(v)"
            @click="newPublished = v"
            :style="{ flex:1, textAlign:'left', padding:'10px 12px', borderRadius:'8px', cursor:'pointer', background: newPublished===v ? 'var(--brand-tint)' : 'var(--bg-0)', border:`1px solid ${newPublished===v ? 'var(--brand-primary)' : 'var(--border-default)'}`, fontFamily:'inherit' }"
          >
    ...
    <template #footer>
      <NButton variant="ghost" size="md" @click="showCreate = false; createError = ''">Отмена</NButton>
      <NButton variant="primary" size="md" :disabled="createLoading" @click="createList">

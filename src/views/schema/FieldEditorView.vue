...
onMounted(async () => {
  ...
  try {
    ...
    fields.value = (t.fields || []).map(f => ({
      name: f.name,
      slug: f.slug,
      type: f.type,
      required: f.required,
      enum_values: f.enum_values || [],
      relation_cardinality: f.relation_cardinality || 'one',
    }))
  } catch (e) {
...
function addField() {
  fields.value.push({ name: '', slug: '', type: 'text', required: false, enum_values: [] })
}

function addRelation() {
  fields.value.push({ name: '', slug: '', type: 'relation', required: false, enum_values: [], relation_cardinality: 'one' })
}
...
function onTypeChange(i, v) {
  ...
  if (v === 'enum' && !fields.value[i].enum_values?.length) {
    fields.value[i].enum_values = []
  }
}
...
async function save() {
  ...
  try {
    const payload = fields.value.map(f => ({
      ...
      required: f.required,
      unique: false,
      enum_values: f.type === 'enum' ? (f.enum_values || []) : undefined,
      relation_cardinality: f.type === 'relation' ? (f.relation_cardinality || 'one') : undefined,
    }))
...
const breadcrumb = computed(() => [
  ...
  'Новая таблица',
  schemaStore.activeTable?.name || tableSlug.value,
])
</script>
      ...
      <template v-else>
        ...
        <div style="display: flex; flex-direction: column; gap: 6px">
          ...
          >
            ...
            <!-- Enum values config -->
            <div v-if="f.type === 'enum'" style="margin-top: 8px; margin-left: 28px; padding: 8px 10px; background: var(--bg-0); border: 0.5px solid var(--border-strong); border-radius: 6px">
              ...
              <input
                ...
                @input="fields[i].enum_values = $event.target.value.split(',').map(v => v.trim()).filter(Boolean)"
            ...
            <div v-if="f.type === 'relation'" style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; row-gap: 8px; margin-top: 8px; margin-left: 28px; padding: 8px 10px; background: var(--bg-0); border: 0.5px solid var(--purple-200); border-radius: 6px">
              ...
              <div style="display: flex; gap: 2px; padding: 2px; background: var(--bg-2); border-radius: 6px">
                <button v-for="[v, l] in [['one', 'одно значение'], ['many', 'список']]" :key="v"
                  @click="fields[i].relation_cardinality = v" :style="{
                    height: '24px', padding: '0 10px', border: 0, borderRadius: '4px', cursor: 'pointer',
                    fontSize: '11px', fontWeight: 500, fontFamily: 'inherit',
                    background: f.relation_cardinality === v ? 'var(--bg-0)' : 'transparent',
                    color: f.relation_cardinality === v ? 'var(--purple-700)' : 'var(--fg-3)',
                    boxShadow: f.relation_cardinality === v ? 'var(--shadow-1)' : 'none',
                  }">{{ l }}</button>
              </div>
            </div>
          </div>
        ...
        <div style="display: flex; gap: 8px; margin-top: 12px">
          <button @click="addField" style="flex: 1; padding: 10px 14px; background: transparent; border: 0.5px dashed var(--border-strong); border-radius: 6px; color: var(--fg-2); font-size: 13px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; font-family: inherit">
            <NIcon name="plus" :size="14" />Добавить поле
          </button>
          <button @click="addRelation" style="flex: 1; padding: 10px 14px; background: var(--brand-tint); border: 0.5px dashed var(--purple-300); border-radius: 6px; color: var(--purple-700); font-size: 13px; font-weight: 500; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; font-family: inherit">
            ↔ Добавить связь
          </button>
        </div>

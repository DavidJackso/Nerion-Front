...
async function proceed() {
  ...
  error.value = ''
  if (!tableName.value.trim()) { error.value = 'Введи название таблицы'; return }
  loading.value = true
  try {
    ...
    router.push({ name: 'schema-fields', params: { slug: slug.value, table: t.slug } })
  } catch (e) {
...
const ICONS = { teachers: 'users', courses: 'file', clients: 'box', leads: 'table', staff: 'users', blank: 'plus' }
function tIcon(t) { return ICONS[t.id] || 'table' }

const breadcrumb = computed(() => [space.value?.name || slug.value, 'Новая таблица'])
</script>
  ...
  <AppShell :breadcrumb="breadcrumb">
    <div style="max-width: 860px; margin: 0 auto; padding: 40px 32px 80px">
      ...
      <div v-else style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 28px">
        <!-- Backend templates -->
        <div
          v-for="t in schemaStore.templates" :key="t.id"
          @click="pick(t)"
          :style="{
            background: 'var(--bg-0)',

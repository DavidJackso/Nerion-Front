...
onMounted(async () => {
  try {
    members.value = await membersApi.listMembers(slug.value)
  } catch (e) {
    show(e.message)
  }
})
...
async function invite() {
  error.value = ''
  try {
    await membersApi.inviteMember(slug.value, inviteEmail.value)
...
async function remove(userId) {
  try {
    await membersApi.removeMember(slug.value, userId)
...
function isMe(m) { return m.user_id === auth.user?.id }
  ...
  <AppShell :breadcrumb="breadcrumb">
    ...
    <div style="max-width: 860px; margin: 0 auto; padding: 32px 32px 80px">
      ...
      <div style="background: var(--bg-0); border: 0.5px solid var(--border-default); border-radius: 8px; overflow: hidden">
        ...
        <div
          v-for="(m, i) in members" :key="m.user_id"
          :style="{
